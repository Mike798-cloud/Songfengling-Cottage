...
function renderHotspots() {
  ...
  if (state.scene === "door") {
    ...
    root.innerHTML = [
      ...
      button("按顺序点亮三盏煤油灯", "lampHint", state.deepEggs.lamps),
    ...
    root.innerHTML = [
      ...
      button("试试地图坐标 1-3-7", "map137", state.deepEggs.map137),
    ...
    root.innerHTML = [
      ...
      button("多敲几下青砖", "brickNote", state.deepEggs.brickNote),
      button("翻 1987 年捐款收据", "donation", state.deepEggs.donation)
    ].join("");
  } else {
    root.innerHTML = [
...
function renderDeskPuzzle(root) {
  ...
  root.innerHTML = `
    <p>主屋桌面需要先拼回日记，再整理汇款单。玩法不再是“系统修复”，而是像真实整理遗物一样逐件确认。</p>
    ...
    <div class="paper-note">正确顺序会拼出：“复员回乡，答应班长的事，一定要做到。”</div>
    ...
    ${receiptDone ? '<button class="ghost-btn" data-action="goChest">去炕边木箱</button>' : ""}
  `;
}
...
function renderChestPuzzle(root) {
  root.innerHTML = `
    ...
    <button class="primary-btn" data-action="decodeSmoke" ${state.chestOpened ? "" : "disabled"}>译出烟盒暗号</button>
    ${state.smokeDecoded ? '<button class="ghost-btn" data-action="goCabinet">去里间壁柜</button>' : ""}
  `;
}
...
function renderCabinetPuzzle(root) {
  ...
  root.innerHTML = `
    <p>壁柜有三只小锁：日记封皮、旧军衣暗扣、松动青砖里各藏着一把钥匙。三把钥匙一起转，柜门才肯开。</p>
    <button class="primary-btn" data-action="openCabinet" ${keyReady ? "" : "disabled"}>三钥同转</button>
    <div class="paper-note">${keyReady ? "柜门内侧露出陈铁柱烈士证明、半封信和几张陈家照片。" : `已找到钥匙 ${state.keys.length}/3`}</div>
    <hr />
    <p>把遗书里的三句嘱托，配回对应照片。</p>
    ...
    <button class="primary-btn" data-action="checkMatches" ${state.cabinetOpened ? "" : "disabled"}>贴回照片背面</button>
    ${state.finalUnlocked ? '<button class="ghost-btn" data-action="goAltar">去窗前供桌</button>' : ""}
  `;
}
